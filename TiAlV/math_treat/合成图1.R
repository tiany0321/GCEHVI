...
# 文件路径
# =======================================================
data_folder <- "D:\\work\\CMOBO\\manuscript-ty1\\Figure\\code\\TiAlV\\repeat"
...
# 左图：散点图
# =======================================================
data <- read.csv(file.path(data_folder,"41467_2025_56267_MOESM3_ESM.csv"))
data <- data[,-c(1:2)]
colnames(data)[6:7] <- c("Elongation","UTS")

tar_point00 <- c(16.5, 1190)
special_point <- data.frame(Elongation=tar_point00[1], UTS=tar_point00[2])


ref_es1 <- 0.4 * (max(data$Elongation) - min(data$Elongation)) + min(data$Elongation)

ref_es2 <- (max(data$UTS) - min(data$UTS)) * 0.4+min(data$UTS)
...
ggsave(
  "D:\\work\\CMOBO\\manuscript-ty1\\Figure\\code\\TiAlV\\math_treat\\TiAlV.png",
  final_plot,
  width=36,
  height=20,
  dpi=300
