...
fn_plot=function(selector,test.response.grid,response.grid,model.result,ref00,tar_point00,train_pareto,pareto_P,choose_p){
  ...
  text(x = 0.1, y = 0.9, labels = paste(selector,as.character(iter),sep="-iter"),cex = 1,font=1.7)
  #par(new=T)
...
fn_pareto_P <- function(crit=NULL,vir_target,train_data,train_pareto,ref00,tar_point00,scale){
  ...
  if(crit!="3points"){
    ...
    if(crit=="given_constrain"){
      ...
      pareto_P00 <- fn_pareto_P(
        crit = "3points_dist",
        vir_target = vir_target,
        train_data = response.grid0,
        train_pareto = train_pareto0,
        ref00 = ref00,
        tar_point00 = tar_point00,
        scale = scale
      )
      ...
      # Use exactly the same constraint generated by 3points_dist
      pareto_P <- pareto_P00
    }
  }
  if(crit=="all_pareto"){
