setwd("C:\\Users\\12758\\Desktop\\GCEHVI\\TiAlV\\repeat")
...
for(selector in sel){
 ...
 ############
  ...
  for(j in 1:(iter_num+1))
  {
    ave.dis[j,2*(selector-1)+1]=sum(dis[j,1:repeat_num])/repeat_num
    if(length(unique(t(as.data.frame(dis[j,1:repeat_num]))))==1)
      ave.dis[1,2*(selector-1)+2]=0 else
    ave.dis[j,2*(selector-1)+2]=t.test(dis[j,1:repeat_num])$conf.int[2]-t.test(dis[j,1:repeat_num])$estimate
  ...
  write.csv(ave.dis,"5ave.dis.csv")
