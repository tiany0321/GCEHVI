setwd("C:\\Users\\12758\\Desktop\\GCEHVI\\poloni\\repeat")
...
#############参考点设置
#tar_point00=c(2.173034,3.264337438)#######point 2507
tar_point00=c(13.372003,0.44187336)
#
iter_num=50#50
repeat_num=100#100
threshold=0
...
n <- 800  # 样本数量
X_lhs <- randomLHS(n, d)  # 生成 [0,1] 区间样本
test.grid <- data.frame(
  x1 = X_lhs[,1] * (pi - (-pi)) + (-pi),  # 映射到 [-pi, pi]
  x2 = X_lhs[,2] * (pi - (-pi)) + (-pi)
)
...
plot(response.grid[,1],response.grid[,2],xlim=c(0,65),ylim=c(0,50))
par(new=T)
plot(nd_df[,1],nd_df [,2] ,xlim=c(0,65),ylim=c(0,50),col="red")
