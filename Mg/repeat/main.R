setwd("D:\\work\\CMOBO\\8.3\\CBOMO\\11.6\\Mg\\limit")
...
ref_es1=(max(test.response.grid[,"es1"])-min(test.response.grid[,"es1"]))*0.5
ref_es2=(max(test.response.grid[,"es2"])-min(test.response.grid[,"es2"]))*0.5
...
for(selector in sel){
 ...
 ############
  ...
  for(seed_num in 1:repeat_num){
    ...
    mf1_0 <- km(~., design = design.grid0, response = response.grid0[,1], nugget.estim = T,covtype = "exp")
    mf2_0 <- km(~., design = design.grid0, response = response.grid0[,2], nugget.estim = T,covtype = "exp")
   ...
   # colnames(design.grid)=c("x1","x2")
    ...
    #  response.grid=anti_join(response.grid,response.grid0)
     ...
     # colnames(response.grid)=obj
      ...
      ################model construction
      mf1 <- km(~., design = design.grid0, response = response.grid0[,1], nugget.estim = T,covtype = "exp")
      mf2 <- km(~., design = design.grid0, response = response.grid0[,2], nugget.estim = T,covtype = "exp")
      model = list(mf1, mf2)
