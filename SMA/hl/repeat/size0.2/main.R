setwd("C:\\Users\\12758\\Desktop\\GCEHVI\\hl\\repeat")
...
data0=read.csv("dataall_train.csv")

keep.heat=c("Lambda","VEN","delta.x","ou")
keep.hysteresis=c("delta.r","VEN","Lambda")
keep.name=c("Lambda","delta.x","VEN","ou","delta.r")
keep.x=keep.name
keep.y=obj

test.grid <- data0[,keep.x]
colnames(test.grid)=keep.x

response.grid<- data0[,keep.y]
...
#########虚拟空间构造
######################################
noiselevel_heat=0.07*(max(response.grid[,"es2"])-min(response.grid[,"es2"]))
noiselevel_hysteresis=0.03*(max(response.grid[,"es1"])-min(response.grid[,"es1"]))
...
plot(response.grid[,1],response.grid[,2],xlim=c(0,100),ylim=c(-40,0))
par(new=T)
plot(nd_df[,1],nd_df [,2],xlim=c(0,100),ylim=c(-40,0),col="red")
...
remain_num =which(test.response.grid[,"es1"]>35&test.response.grid[,"es2"]>-30)
...
for(selector in sel){
 ...
 ############
  ...
  for(seed_num in 1:repeat_num){
    ...
    set.seed(243)
    mf2_0 <- km(formula2, design = design.grid0, response = response.grid0[,2],noise.var=noise_heat,covtype = "exp")
    
    formula1<- es1~ delta.r + VEN + Lambda
    noise_hysteresis=rep(noiselevel_hysteresis^2,nrow(response.grid0))
    set.seed(243)
    mf1_0 <- km(formula1, design = design.grid0, response = response.grid0[,1],noise.var=noise_hysteresis,covtype = "exp")
   ...
   # colnames(design.grid)=c("x1","x2")
    ...
    #  response.grid=anti_join(response.grid,response.grid0)
     ...
     # colnames(response.grid)=obj
      ...
      ################model construction
      noise_heat0=rep(noiselevel_heat^2,nrow(response.grid0))
      set.seed(243)
      mf2 <- km(formula2, design = design.grid0, response = response.grid0[,2],noise.var=noise_heat0,covtype = "exp")
      
      
      noise_hysteresis0=rep(noiselevel_hysteresis^2,nrow(response.grid0))
      set.seed(243)
      mf1 <- km(formula1, design = design.grid0, response = response.grid0[,1],noise.var=noise_hysteresis0,covtype = "exp")
  ...
  for(j in 1:(iter_num+1))
  {
    ave.delta_hv[j,2*(selector-1)+1]=sum(delta_hv[j,1:repeat_num])/repeat_num
    if(length(unique(t(as.data.frame(delta_hv[j,1:repeat_num]))))==1)
      ave.delta_hv[1,2*(selector-1)+2]=0 else
        ave.delta_hv[j,2*(selector-1)+2]=t.test(delta_hv[j,1:repeat_num])$conf.int[2]-t.test(delta_hv[j,1:repeat_num])$estimate
      # sd(data.ave.SD[j,1:i])/2
      
  }
  for(j in 1:(iter_num+1))
  {
    ave.delta_hv_given[j,2*(selector-1)+1]=sum(delta_hv_given[j,1:repeat_num])/repeat_num
    if(length(unique(t(as.data.frame(delta_hv_given[j,1:repeat_num]))))==1)
      ave.delta_hv_given[1,2*(selector-1)+2]=0 else
        ave.delta_hv_given[j,2*(selector-1)+2]=t.test(delta_hv_given[j,1:repeat_num])$conf.int[2]-t.test(delta_hv_given[j,1:repeat_num])$estimate
    #   sd(data.ave.SD[j,1:i])/2
