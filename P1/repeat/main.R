setwd("C:\\Users\\12758\\Desktop\\GCEHVI\\P1\\repeat")
...
#############参考点设置
tar_point00=c(9.5131893, -24.98376)
#
iter_num=50
repeat_num=100
threshold=0
...
P1=function (x1,x2) {
  ...
  b1 <- 15 * x1 - 5
  b2 <- 15 * x2
  y1=(b2 - 5.1 * (b1/(2 * pi))^2 + 5/pi * b1 - 6)^2 + 10 * ((1 - 1/(8 * pi)) * cos(b1) + 1)
  y2=-sqrt((10.5 -  b1) * (b1 + 5.5) * (b2 + 0.5)) - 1/30 * (b2 - 5.1 * (b1/(2 * pi))^2 - 6)^2 - 1/3 * ((1 - 1/(8 * pi)) * cos(b1) + 1)
  return(data.frame(x1 = x1, x2 = x2, y1 = y1, y2 = y2))
}
...
size=0.05
#########虚拟空间构造
test.grid <- expand.grid(
  x1 = seq(0, 1, by = 0.05),
  x2 = seq(0, 1, by = 0.05)#,
  # x3 = seq(0, 1, by = 0.1)
)
n.grid <- round(nrow(test.grid)*0.05)#26

colnames(test.grid)=keep.x
n_appr <- round(size*nrow(test.grid))

#write.csv(test.grid,"test.grid.csv")
data=do.call(f_name, test.grid)
#data=OKA1(test.grid) 
######################################
colnames(data)=c(keep.x,keep.y)
response.grid <-data[,keep.y] #as.data.frame(t(apply(test.grid, 1, f_name)))
...
colnames(response.grid)=keep.y
plot(response.grid[,1],response.grid[,2],xlim=c(0,300),ylim=c(-35,-5))
par(new=T)
plot(nd_df[,1],nd_df [,2] ,xlim=c(0,300),ylim=c(-35,-5),col="red")
  ...
  #  response.grid=response.grid0
   # colnames(design.grid)=c("x1","x2")
    ...
    for(iter in 1:iter_num){
      ...
      new.design.grid=as.data.frame(test.grid[choose_p,])
